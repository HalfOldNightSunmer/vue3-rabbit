...
import { getCheckoutInfoAPI, createOrderAPI, getOrderDetailAPI } from '@/apis/order'
const checkInfo = ref({}) // 订单对象
const curAddress = ref({})  // 地址对象
const summary = ref({}) // 订单价格
...
const submit = () => {
    const item = checkInfo.value.userAddresses.find(item => item.id === index.value)
...
const submitOrder =  async () => {
    console.log('提交订单地址:', curAddress.value)
    const requestBody = {
    deliveryTimeType: 1,
    payType: 1,
    payChannel: 1,
    ...
    const res = await createOrderAPI(requestBody)
