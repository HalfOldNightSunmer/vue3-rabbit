...
const { remainTime, getTime } = useCountDown()
...
// 支付宝账号：fukuvb7569@sandbox.com
// 密码：111111

const getOrderDetail = async () => { 
    const res = await getOrderDetailAPI(route.params.id)
    console.log('获取订单结果：',res.data.result)
    payInfo.value = res.data.result
}

onMounted(async () => { 
    await getOrderDetail()
    getTime(payInfo.value.countdown)
})
  ...
  <div class="xtx-pay-page">
    <div class="container">
      ...
      <div class="pay-info">
        ...
        <div class="tip">
          <p>订单提交成功！请尽快完成支付。</p>
          <p>支付还剩 <span>{{remainTime}}</span>, 超时后将取消订单</p>
        </div>
        <div class="amount">
          <span>应付总额：</span>
